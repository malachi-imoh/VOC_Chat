...
def normalize_header(h: str) -> str:
    """Normalize a CSV header by stripping punctuation, whitespace, and lowering case."""
    ...
    def create_database(self, csv_path: str):
        ...
        try:
            ...
            for q_type, type_info in self.question_types.items():
                ...
                for expected_col in question_columns:
                    norm_expected = normalize_header(expected_col)
                    matches = [c for c in df.columns if normalize_header(c) == norm_expected]


                if not matches:
                    close = difflib.get_close_matches(expected_col, df.columns, n=1, cutoff=0.6)
                    if close:
                        matches = [close[0]]
                
                if not matches:
                    print(f"{Fore.YELLOW}Skipping column not found in CSV: {expected_col}{Style.RESET_ALL}")
                    continue
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Create VOC ChromaDB database from CSV")
    parser.add_argument("--csv", type=str, required=True, help="Path to input CSV file")
    parser.add_argument("--persist_dir", type=str, required=True, help="Directory for ChromaDB persistence")
    parser.add_argument("--do-clustering", action="store_true", help="Enable clustering with DBSCAN")
    parser.add_argument("--chunk_tokens", type=int, default=0, help="Chunk size in tokens (0 = no chunking)")
    ...
    creator.create_database(args.csv)
