...
class ChromaToPineconeMigrator:
    ...
    def retrieve_chroma_data(self):
        ...
        # Print only one sample record instead of full debug dump
            if results.get("documents") and results.get("metadatas"):
                print(f"\n{Fore.YELLOW}Sample record from ChromaDB:{Style.RESET_ALL}")
                print(f"  Document: {results['documents'][0][:200]}...")  # first 200 chars
    ...
    def upload_to_pinecone(self, vectors: List[Dict], batch_size: int = None):
        ...
        try:
            ...
            try:
                ...
                print(f"\n{Fore.YELLOW}Pinecone index stats after upload:{Style.RESET_ALL}")
                print(f" Total vectors in index: {total}")
            except Exception as e:
                print(f'{Fore.RED}Coult not retrieve Pinecone stats: {e}{Style.RESET_ALL}')
...
def main():
    ...
    parser.add_argument(
        '--chroma-dir',
        type=str,
        default="../chroma_db_q3_2025",   # Needs to match upload
        help='Path to ChromaDB persistence directory'
    )
    parser.add_argument(
        '--pinecone-api-key',
        type=str,
        default = get_secret("PINECONE_API_KEY"),   # load from .env
    ...
    parser.add_argument(
        '--index-name',
        type=str,
        default="voc-index-2025-q3",   # neeeds to match the app.py script
